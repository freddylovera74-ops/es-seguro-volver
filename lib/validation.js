...
export const MAX_PHOTOS = 20;              // máximo de fotos por vivienda
...
// Acepta un array de data-URLs (o uno solo) y devuelve hasta MAX_PHOTOS
// fotos decodificadas, descartando las inválidas.
export function decodePhotos(input) {
  const arr = Array.isArray(input) ? input : input ? [input] : [];
  const out = [];
  for (const dataUrl of arr.slice(0, MAX_PHOTOS)) {
    const p = decodePhoto(dataUrl); // puede lanzar HttpError si una es muy grande
    if (p) out.push(p);
  }
  return out;
}
