...
export async function getVivienda(env, id, { includeHidden = false } = {}) {
  ...
  const opSql = `SELECT id, nombre, profesion, civ, status, comentario, verified, hidden, reports, created_at
...
export async function createVivienda(env, b, fotoPath) {
  ...
  await env.DB.prepare(
    `INSERT INTO viviendas (id,nombre,contacto,ubicacion,sector,tipo,pisos,piso,construccion,damages,descripcion,foto,created_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).bind(
    row.id, row.nombre, row.contacto, row.ubicacion, row.sector, row.tipo,
    row.pisos, row.piso, row.construccion, row.damages, row.descripcion, row.foto, row.created_at
  ).run();
  return id;
}

export async function setFoto(env, id, fotoPath) {
  await env.DB.prepare('UPDATE viviendas SET foto = ? WHERE id = ?').bind(fotoPath, id).run();
}

export async function createOpinion(env, vid, b) {
  await env.DB.prepare(
    `INSERT INTO opinions (vivienda_id,nombre,profesion,civ,status,comentario,created_at)
     VALUES (?,?,?,?,?,?,?)`
  ).bind(
    vid, clip(b.nombre, 80), clip(b.profesion, 80), clip(b.civ, 30), b.status, clip(b.comentario, 2000), Date.now()
  ).run();
}
